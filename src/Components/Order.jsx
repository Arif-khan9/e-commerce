...
const Order = () => {
  ...
  const images = Array.isArray(card.images) ? card.images: [card.thumbnail || card.image];
  ...
  return (
   ...
   <>
   <div > 
   <button className="px-12 p-2 bg-amber-300 rounded-full mt-15 ml-15 hover:bg-amber-200"  onClick={() => window.history.back()}><ChevronLeft className="w-6 h-6 black" /></button>
   </div>
    <div className="flex justify-center items-center h-150 gap-15 md:flex-col lg:flex-row flex-col">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg max-w-lg">
        ...
        <div className="flex flex-col items-start mt-5">
          <button className="px-8 py-3 bg-blue-700 text-white rounded-full hover:bg-blue-600 transition">
           Add to Cart
          </button>
        </div>
      </div>
